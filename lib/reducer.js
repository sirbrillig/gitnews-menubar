...
function reducer( state, action ) {
	...
	switch ( action.type ) {
		...
		case 'OFFLINE':
			return Object.assign( {}, state, { offline: true, lastChecked: Date.now(), fetchInterval: getFetchInterval( secsToMs( 30 ), state.fetchRetryCount ), fetchRetryCount: state.fetchRetryCount + 1 } );
