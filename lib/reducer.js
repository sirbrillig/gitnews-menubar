...
function reducer( state, action ) {
	...
	switch ( action.type ) {
		case 'FETCH_BEGIN':
			return Object.assign( {}, state, { fetchingInProgress: true } );
		case 'FETCH_END':
			return Object.assign( {}, state, { fetchingInProgress: false } );
		case 'ADD_CONNECTION_ERROR':
			return Object.assign( {}, state, { errors: [ ...this.state.errors, action.error ], lastChecked: Date.now(), fetchingInProgress: false } );
		case 'CLEAR_ERRORS':
		...
		case 'OFFLINE':
			return Object.assign( {}, state, { offline: true, lastChecked: Date.now(), fetchInterval: getFetchInterval( secsToMs( 60 ), state.fetchRetryCount ), fetchRetryCount: state.fetchRetryCount + 1, fetchingInProgress: false } );
		case 'NOTES_RETRIEVED':
			return Object.assign( {}, state, { offline: false, lastChecked: Date.now(), lastSuccessfulCheck: Date.now(), fetchRetryCount: 0, errors: [], fetchInterval: defaultFetchInterval, notes: mergeNotifications( state.notes, action.notes ), fetchingInProgress: false } );
	}
	return state;
}
